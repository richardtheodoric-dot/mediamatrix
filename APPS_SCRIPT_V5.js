...
function applyColumnWidths_(sheet) {
  ...
  const widthsBySheet = {
    ...
    Clients: [165,220,280,120,430,180,150,150,280,120,360],
...
// ── CLIENTS ───────────────────────────────────────────────────────
function getClients() {
  const sheet = sh('Clients', ['id','name','platforms_json','budget','docsUrl','createdAt','pic','niche','notes','contractMonths','resources_json']);
  const rows = sheet.getDataRange().getValues();
  ...
  return {
    clients: rows.slice(1).map(r => ({
      ...
      pic:r[6]||'', niche:r[7]||'', notes:r[8]||'', contractMonths:r[9]||'', resources: JSON.parse(r[10]||'[]')
    }))
  };
}
...
function saveClients(clients) {
  const sheet = sh('Clients', ['id','name','platforms_json','budget','docsUrl','createdAt','pic','niche','notes','contractMonths','resources_json']);
  const headers = ['id','name','platforms_json','budget','docsUrl','createdAt','pic','niche','notes','contractMonths','resources_json'];
  ...
  const rows = (clients || []).map(c => {
    ...
    return [
      ...
      c.pic||'', c.niche||'', c.notes||'', c.contractMonths||'', JSON.stringify(c.resources||[])
    ];
  });
  sheet.clearContents();
...
function renderAllAdsCampaignReports_() {
  ...
  renderObjectiveAdsReportSheets_(ss, reports);
  updatedTabs.push('Marketplace Ads', 'Meta Ads (Profile Visits)', 'Meta Ads (LP Views)', 'Meta Ads (WhatsApp)');
...
function renderObjectiveAdsReportSheets_(ss, reports) {
  const byObjective = { marketplace: [], meta_profile: [], meta_lpv: [], meta_leads: [] };
  (reports || []).forEach(rep => {
    const sections = rep.data?.sections || {};
    Object.keys(sections).forEach(id => {
      const section = sections[id] || {};
      const objective = section.objective || inferObjective_(section.platform, section.kpi);
      if (!byObjective[objective]) byObjective[objective] = [];
      byObjective[objective].push({
        clientName: rep.clientName,
        periodLabel: rep.periodLabel || formatPeriodLabel(rep.periodKey),
        section
      });
    });
  });
  renderMarketplaceSheet_(ss, byObjective.marketplace || []);
  renderMetaSheet_(ss, 'Meta Ads (Profile Visits)', 'META (PROFILE VISITS)', 'Profile Visits', 'Cost Per Visit', byObjective.meta_profile || []);
  renderMetaSheet_(ss, 'Meta Ads (LP Views)', 'META (LANDING PAGE VIEWS)', 'Landing Page Views', 'Cost Per Landing Page View', byObjective.meta_lpv || []);
  renderLeadsSheet_(ss, byObjective.meta_leads || []);
}
